  ...
  - name: ${cluster.tag}-kubernetes-metrics
    rules:
      ...
      - record: ${replace(cluster.tag, "-", "_")}_kubernetes:containers_cpu_request_usage:percentage
        expr: 100 * max by (namespace, pod, container) (rate(container_cpu_usage_seconds_total[5m]) / on (namespace, pod, container) kube_pod_container_resource_requests{resource="cpu", cluster="${cluster.tag}"})
      - record: ${replace(cluster.tag, "-", "_")}_kubernetes:containers_cpu_limit_usage:percentage
        expr: 100 * max by (namespace, pod, container) (rate(container_cpu_usage_seconds_total[5m]) / on (namespace, pod, container) kube_pod_container_resource_limits{resource="cpu", cluster="${cluster.tag}"})
      ...
      - record: ${replace(cluster.tag, "-", "_")}_kubernetes:containers_memory_request_usage:percentage
        expr: 100 * max by (namespace, pod, container) (container_memory_working_set_bytes / on (namespace, pod, container) kube_pod_container_resource_requests{resource="memory", cluster="${cluster.tag}"})
      - record: ${replace(cluster.tag, "-", "_")}_kubernetes:containers_memory_limit_usage:percentage
        expr: 100 * max by (namespace, pod, container) (container_memory_working_set_bytes / on (namespace, pod, container) kube_pod_container_resource_limits{resource="memory", cluster="${cluster.tag}"})
...
%{ for service in cluster.expected_services ~}
      - record: ${replace(cluster.tag, "-", "_")}_${replace(service.name, "-", "_")}_running_pods:count
        expr: sum by () (kube_pod_status_phase{phase="Running", pod=~"${service.name}[-][a-z0-9]+[-][a-z0-9]+", namespace="${service.namespace}"} and on(pod, namespace) (container_start_time_seconds > ${service.expected_start_delay})) or vector(0)
